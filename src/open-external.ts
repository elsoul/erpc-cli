...
// changes its quoting, `%NAME%` expands an environment variable, and a
// string that is not a URL can start a program. So on Windows only an
// http(s) URL whose serialized form stays inside a conservative subset of
// URL characters is handed over. Any other URL is not opened; every caller
// prints the URL before it calls the launcher.

/**
 * URL characters that cmd reads as plain text. `%` is left out as well, so
 * neither a percent-encoded byte nor `%NAME%` reaches cmd.
 */
